...
window.priceIntelligence = (function() {
    ...
    function checkDependencies() {
        ...
        if (missing.length > 0) {
            console.error(`[PriceIntelligence] Dependências ausentes: ${missing.join(', ')}`);
            return false;
        }
        return true;
    }
            ...
            this.instances.comparison = new dependencies.Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: ctx => `R$ ${dependencies.utils.formatCurrency(ctx.raw)}`
                            }
                        }
                    },
                    scales: {
            ...
            this.instances.stockComparison = new dependencies.Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: ctx => `${ctx.dataset.label}: R$ ${dependencies.utils.formatCurrency(ctx.raw)}`
                            }
                        }
                    },
                    scales: {
    ...
    const UI = {
        ...
        attachEvents() {
            ...
            if (mainContent) {
                ...
                // Input events para preço de mercado (delegação)
                        ...
                        <div class="card-modern">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" id="manual-price" class="form-control"
                                           placeholder="0,00"
        ...
        renderResults(results, query, source) {
            ...
            body.innerHTML = results.map((item, index) => {
                ...
                return `
                    <tr>
                        <td>${title}</td>
                        <td class="text-primary fw-bold">R$ ${dependencies.utils.formatCurrency(price)}</td>
                        <td>${store}</td>
                        <td>
            ...
            body.innerHTML = [...history]
                ...
                .map(item => {
                    ...
                    return `
                        <tr>
                            <td>${safeName}</td>
                            <td class="text-primary fw-bold">R$ ${dependencies.utils.formatCurrency(item.price)}</td>
                            <td>${safeStore}</td>
        ...
        async linkToProduct(index) {
            ...
            const options = '<option value="">Selecione...</option>' +
                products.map(p =>
                    `<option value="${this.escapeHtml(p.id)}">${this.escapeHtml(p.nome)} (R$ ${dependencies.utils.formatCurrency(p.preco)})</option>`
                ).join('');
            ...
            const { value: productId } = await dependencies.Swal.fire({
                ...
                html: `
                    <div class="text-start">
                        ...
                        <p><strong>Preço:</strong> R$ ${dependencies.utils.formatCurrency(item.price)}</p>
                        <hr>
            ...
            if (product) {
                const { isConfirmed } = await dependencies.Swal.fire({
                    ...
                    text: `Preço atual: R$ ${dependencies.utils.formatCurrency(product.preco)} → Novo: R$ ${dependencies.utils.formatCurrency(item.price)}`,
                    icon: 'question',
                    showCancelButton: true,
        ...
        showComparisonGraph(product, marketItem) {
            ...
            if (ourPriceEl) ourPriceEl.textContent = `R$ ${dependencies.utils.formatCurrency(product.preco)}`;
            ...
            if (marketAvgEl) marketAvgEl.textContent = `R$ ${dependencies.utils.formatCurrency(marketAvg)}`;
            if (suggestedEl) suggestedEl.textContent = `R$ ${dependencies.utils.formatCurrency(suggested)}`;
        ...
        autoLinkFirstResult(productId, results) {
            ...
            // Atualiza campo do painel de estoque
            const input = document.getElementById(`market-input-${productId}`);
            if (input) {
                input.value = dependencies.utils.formatCurrency(item.price);
                this.onMarketPriceInput(productId);
            }
        },
            ...
            return products.map(p => {
                ...
                if (diff !== null) {
                    ...
                    diffBadge = `
                        <span class="badge bg-${cls}">
                            ${icon} R$ ${dependencies.utils.formatCurrency(Math.abs(diff))}
                ...
                return `
                    <tr id="stock-row-${this.escapeHtml(p.id)}">
                        ...
                        <td class="fw-bold">R$ ${dependencies.utils.formatCurrency(p.preco)}</td>
                        <td>
                        ...
                        <td id="suggested-cell-${this.escapeHtml(p.id)}">
                            ${suggested !== null
                                ? `<span class="text-success fw-bold">R$ ${dependencies.utils.formatCurrency(suggested)}</span>`
        ...
        onMarketPriceInput(productId) {
            ...
            const marketPrice = parseFloat(rawVal) || 0;
            ...
            if (marketPrice > 0) {
                ...
                if (diffCell) {
                    diffCell.innerHTML = `
                        <span class="badge bg-${cls}">
                            ${icon} R$ ${dependencies.utils.formatCurrency(Math.abs(diff))} (${Math.abs(diffPct).toFixed(1)}%)
                        </span>`;
                }
                if (suggestedCell) {
                    suggestedCell.innerHTML = `<span class="text-success fw-bold">R$ ${dependencies.utils.formatCurrency(suggested)}</span>`;
                }
            } else {
        ...
        exportStockComparison() {
            ...
            const data = products.map(p => {
                ...
                return {
                    'Produto': p.nome,
                    'Código': p.code || '',
                    'Nosso Preço (R$)': dependencies.utils.formatCurrency(p.preco),
                    'Preço de Mercado (R$)': market > 0 ? dependencies.utils.formatCurrency(market) : 'Não informado',
                    'Diferença (R$)': diff !== null ? dependencies.utils.formatCurrency(diff) : '',
                    'Diferença (%)': diffPct !== null ? diffPct.toFixed(1) + '%' : '',
                    'Preço Sugerido (R$)': suggested !== null ? dependencies.utils.formatCurrency(suggested) : ''
                };
            });
        ...
        exportHistory() {
            ...
            const data = history.map(h => ({
                ...
                'Preço': dependencies.utils.formatCurrency(h.price),
                'Loja': h.store,
